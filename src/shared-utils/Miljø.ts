...
const Miljø = (): MiljøProps => {
    if (erDev()) {
        return {
            ...
            wonderwallUrl:
                'https://familie-ks-soknad.ekstern.dev.nav.no/familie/kontantstotte/soknad/oauth2/login?redirect=',
            oauthCallbackUri:
                'https://familie-ks-soknad.ekstern.dev.nav.no/familie/kontantstotte/soknad/oauth2/callback',
            port: 9000,
        };
    } else if (erProd()) {
        return {
